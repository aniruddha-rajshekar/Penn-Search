...
#define APP_MODULE_NAME "PENNSEARCH"
...
class SimulatorMain
{
  ...
  private:
    Ptr<RealtimeSimulatorImpl> m_rtImpl;
...
void
SimulatorMain::SetRealStack (bool realStack)
{
...
SimulatorMain::Start (std::string scriptFile, NodeContainer nodeContainer, NetDeviceContainer* ndc, uint32_t totalNodes, uint32_t totalLinks)
{
  ...
  m_rtImpl = DynamicCast<RealtimeSimulatorImpl> (Simulator::GetImplementation ());
  ...
  while (1)
    {
      ...
      if (commandLine == "")
        continue;
      if (simulatorMain->m_readyToRead == true && simulatorMain->m_realStack == false)
        {
      ...
      if (!simulatorMain->m_realStack)
        ...
        {
          // Insert event 
          simulatorMain->m_rtImpl->ScheduleRealtimeNowWithContext (0, MakeEvent (&SimulatorMain::ProcessCommandTokens, simulatorMain, simulatorMain->m_tokens, MilliSeconds (0.0)));
          simulatorMain->m_tokens.clear ();
        }
    }
  Simulator::Stop ();
  pthread_exit (NULL);
  ...
  // Check for Link Command
  ...
  else
    {
      if (!m_realStack)
        ...
        {
          // Send to node 0
          ProcessNodeCommandTokens (0, tokens, time);
        }
    }
}
...
main (int argc, char *argv[])
{
  ...
  for (uint32_t i = 0 ; i < totalNodes ; i++)
    {
      ...
      if (realStack == "YES")
      {
    	  application->SetRealStack (true);
        application->SetNodeId (nodeName);
        application->SetLocalAddress (local);
      }
      else
      {
  ...
  apps.Start (MilliSeconds (START_TIME));

  NS_LOG_INFO ("Creating script/command handler...");  
  // Start simulator main
  if (realStack == "YES")
    {
      simulatorMain.SetRealStack (true);
    }
