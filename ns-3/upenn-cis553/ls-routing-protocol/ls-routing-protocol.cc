...
LSRoutingProtocol::Flooding ()
{
    ...
    //TRAFFIC_LOG ("Sending LSP" << " Message: " << message << " SequenceNumber: " << seqno);
    Ptr<Packet> packet = Create<Packet> ();
...
LSRoutingProtocol::ProcessLsp (LSMessage lsMessage,Ptr<Socket> socket)
{
    ...
    //TRAFFIC_LOG ("Received LSP, From Node: " << fromNode<<" with LSP Sequence number: " << lsMessage.GetLsp().sequenceNumber<< " "<< ", Message: " << message);
