...
DVRoutingProtocol::GetTypeId (void)
{
  ...
  .AddAttribute ("NdTimeout",
                ...
                TimeValue (MilliSeconds (2000)),
                MakeTimeAccessor (&DVRoutingProtocol::m_ndTimeout),
                 MakeTimeChecker ())
  .AddAttribute ("SingleHop",
                "Single Hop TTL Value",
                UintegerValue (1),
                MakeUintegerAccessor (&DVRoutingProtocol::m_singleHop),
...
DVRoutingProtocol::DoStart ()
{
  ...
  m_ndTimer.SetFunction (&DVRoutingProtocol::NdRequests, this);
  // Start timers
...
DVRoutingProtocol::DumpNeighbors ()
{
  ...
  {
    PRINT_LOG (std::endl<<"\t"<< iter->first <<"\t\t"<< iter->second.neighborAddr <<"\t\t"<<iter->second.interfaceAddr);
  }
  ...
  switch (dvMessage.GetMessageType ())
    {
      ...
      case DVMessage::PING_RSP:
        ...
        break;
      case DVMessage::ND_REQ:
        ProcessNdReq (dvMessage);
        break;
      case DVMessage::ND_RSP:
        ProcessNdRsp (dvMessage, socket);
        break;
      default:
...
void
DVRoutingProtocol::NeighborDiscovery ()
{
    //TRAFFIC_LOG ("NeighborDiscovery called");
    ...
    {
       uint32_t sequenceNumber = GetNextSequenceNumber ();
       //TRAFFIC_LOG ("Sending ND_REQ to Node: " << nodeNumber << " IP: " << destAddress << " Message: " << pingMessage << " SequenceNumber: " << sequenceNumber);
       Ptr<Packet> packet = Create<Packet> ();
       DVMessage dvMessage = DVMessage (DVMessage::ND_REQ, sequenceNumber, m_singleHop, m_mainAddress);
       dvMessage.SetNdReq (destAddress, pingMessage);
       packet->AddHeader (dvMessage);
       BroadcastPacket (packet);
    }
}
...
void
DVRoutingProtocol::ProcessNdReq (DVMessage dvMessage)
{
  // Use reverse lookup for ease of debug
  std::string fromNode = ReverseLookup (dvMessage.GetOriginatorAddress ());
  //TRAFFIC_LOG ("Received ND_REQ, From Node: " << fromNode << ", Message: " << dvMessage.GetPingReq().pingMessage);
  // Send ND Response
  DVMessage dvResp = DVMessage (DVMessage::ND_RSP, dvMessage.GetSequenceNumber(), m_singleHop, m_mainAddress);
  dvResp.SetNdRsp (dvMessage.GetOriginatorAddress(), dvMessage.GetPingReq().pingMessage);
  Ptr<Packet> packet = Create<Packet> ();
  packet->AddHeader (dvResp);
  BroadcastPacket (packet);
...
void
DVRoutingProtocol::ProcessNdRsp (DVMessage dvMessage,Ptr<Socket> socket)
{
  // Check destination address
  if (IsOwnAddress (dvMessage.GetPingRsp().destinationAddress))
    {
      ...
      std::map <Ipv4Address, uint32_t>::iterator it = m_addressNodeMap.find(dvMessage.GetOriginatorAddress());
      uint32_t nodeNumber = it->second;
      NeighborTableEntry neighborTableEntry =  (NeighborTableEntry) {dvMessage.GetOriginatorAddress(),InterfaceAddr};
      m_neighborTable[nodeNumber]=neighborTableEntry;
      //TRAFFIC_LOG ("Received ND_RSP, From Node: " << nodeNumber << " IPAdress : " << neighborTableEntry.neighborAddr << ", Message: " << dvMessage.GetPingRsp().pingMessage <<" Interface Address : " << neighborTableEntry.interfaceAddr);
    }
}
